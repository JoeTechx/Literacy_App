...
import { Users, BookOpen, PlusCircle, LogOut, X, LayoutDashboard, ShieldAlert } from 'lucide-react';
import { teacherAPI, adminAPI, uploadAPI } from '@/utils/api';
...
export default function Dashboard() {
  ...
  // Students should not be using the web dashboard
  if (isStudent) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        ...
        >
          ...
          <h1 className="text-2xl font-bold text-foreground mb-3">Wrong Place!</h1>
          <p className="text-muted-foreground mb-6">
            Hi <span className="text-foreground font-semibold">{user.name}</span>! This is the teacher web portal. Students use the <strong className="text-amber-400">mobile app</strong> to access their learning modules.
          </p>
          <p className="text-sm text-muted-foreground mb-8">
            Please ask your teacher to help you open the Literacy App on your tablet or phone.
          </p>
          <button
            onClick={handleLogout}
            className="w-full py-3 bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 font-semibold rounded-xl border border-amber-500/30 transition-colors cursor-pointer"
          >
            Log Out
          </button>
        </motion.div>
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <main className="flex-1 p-10 overflow-y-auto relative">
        {/* Ambient Glow */}
        {isAdmin && <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-blue-500/10 rounded-full mix-blend-screen filter blur-[120px] pointer-events-none"></div>}
        ...
        <header className="flex justify-between items-center mb-10 relative z-10">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              {isAdmin ? 'Manage the entire platform ecosystem.' : 'Manage your classroom and curriculum.'}
            </p>
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'admin_modules' && isAdmin && (
      ...
      {/* Create Module Modal (Teacher Only) */}
      <AnimatePresence>
        {isModalOpen && !isAdmin && (
